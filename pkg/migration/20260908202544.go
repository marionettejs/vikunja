...
func backfillProjectAncestors20260908202544(tx *xorm.Engine) error {
	...
	rows := make([]*projectAncestors20260908202544, 0, len(allProjects))
	for _, project := range allProjects {
	...
	for chunk := range slices.Chunk(rows, projectAncestorsBatch20260908202544) {
		if _, err := tx.Insert(chunk); err != nil {
...
func init() {
	migrations = append(migrations, &xormigrate.Migration{
		...
		Rollback: func(tx *xorm.Engine) error {
			return tx.DropTables(projectAncestors20260908202544{})
		},
	})
}
